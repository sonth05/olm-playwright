 ...
 *
 * Mỗi test (TC-05,06 và TC-10,12,13) tự mở lại modal từ đầu bằng test.step()
 * riêng, nhưng dùng CHUNG 1 page/browser context trong cùng 1 test — không mở
 * lại getPageAsRole() cho từng ca nhỏ. 2 test-block độc lập với nhau (mỗi
 * test có page riêng), nhưng các step BÊN TRONG 1 test chạy tuần tự và có
 * dọn dẹp (đóng modal) trước khi sang step kế tiếp.
 */
test.describe('[FUNCTION] TC-EXAM-MODAL: Modal "Tạo Đề kiểm tra"', () => {
  ...
  test('TC-EXAM-MODAL-10, 12, 13: Hành vi đóng/không đóng modal', async ({ getPageAsRole }) => {
    const page = await getPageAsRole('editableTeacher');
    const listPage = new HocLieuCuaToiV2Page(page);
    await listPage.goto();
    const menu = new CreateHocLieuMenu(page);
