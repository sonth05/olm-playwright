...
test.describe('TinTuc /thongtin @tin-tuc', () => {
...
test.describe('TinTuc /hoc-tap @tin-tuc', () => {
	...
	test('[Happy] Link bài viết trên /hoc-tap dùng /tin-tuc/ prefix', async ({ page }) => {
				const p = new TinTucPage(page);
				await p.openHocTap();
				const tinTucLinks = await page.locator("a.olm-text-link[href*='/tin-tuc/']").count();
	...
	test('[Happy] Click bài viết đầu tiên trên /hoc-tap → navigate thành công', async ({ page }) => {
				...
				await p.openHocTap();
				await p.clickFirstArticle();
				expect(p.getCurrentUrl()).toMatch(/\/(bai-viet|tin-tuc)\//);
			});
	...
	test('[Happy] Phân trang có số trang hợp lệ (>= 1)', async ({ page }) => {
				const p = new TinTucPage(page);
				await p.openHocTap();
				expect(await p.getPaginationPageCount()).toBeGreaterThanOrEqual(1);
			});

	test('[Happy] Trang active mặc định là trang 1', async ({ page }) => {
				const p = new TinTucPage(page);
				await p.openHocTap();
				expect(await p.getActivePageNumber()).toBe(1);
			});
	...
	test('[Happy] Click page 2 → URL chứa page-2 và trang active = 2', async ({ page }) => {
				...
				expect(await p.getActivePageNumber()).toBe(2);
			});
...
test.describe('TinTuc /thong-bao @tin-tuc', () => {
