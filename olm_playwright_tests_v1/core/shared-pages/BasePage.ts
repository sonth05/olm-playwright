...
export class BasePage {
  ...
  // được định nghĩa trên BasePage, khiến TypeScript báo lỗi TS2339
  // "Property does not exist" ở toàn bộ các page object đó.)
   ...
   *
   * Nếu trang đã có nội dung rồi, chỉ chạy lại bước dismiss popup (phòng
   * khi có popup mới xuất hiện) mà KHÔNG load lại từ đầu.
   */
  protected async ensurePageLoaded(): Promise<void> {
  ...
  /**
   * Cuộn bên trong sidebar điều hướng để đưa `locator` vào vùng nhìn thấy.
   * Dùng khi sidebar dài (nhiều nhóm menu) và mục cần bấm nằm ngoài
   * viewport ban đầu.
   *
   * Thử `scrollIntoViewIfNeeded()` trước (đủ dùng cho hầu hết trường hợp
   * vì nó tự tìm scroll container gần nhất, kể cả khi đó không phải
   * window). Nếu sau đó locator vẫn chưa visible (VD: sidebar dùng
   * overflow container riêng mà scrollIntoViewIfNeeded() không chạm tới
   * đúng cách do đang bị che/animate), fallback sang cuộn chuột theo
   * bước nhỏ tối đa `maxAttempts` lần.
   */
  protected async scrollSidebarUntilVisible(
    locator: Locator,
    maxAttempts = 10
  ): Promise<void> {
    await locator.scrollIntoViewIfNeeded().catch(() => {});
