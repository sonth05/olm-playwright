 ...
 * URL mẫu: https://olm.vn/lop-10-mon-toan
 *
 * URL thực tế đã xác nhận:
 *   Học bài     → /hoc-bai
 *   Hỏi bài     → /hoi-dap
 *   Kiểm tra    → /contestx
 *   ĐGNL        → dgnl.olm.vn
 *   Thi đấu     → dautruong.olm.vn
 *   Thư viện số → /thu-vien-so
 *   Sidebar     → URL thay đổi theo lớp
 */
test.describe('Header Trang Khóa Học @header-course @regression', () => {
  ...
  test('[Happy] Nav "Thi đấu" điều hướng đúng', async ({ page }) => {
    ...
    // href thực tế là /thi-dau (không phải dautruong.olm.vn)
  ...
  test('[Happy] Sidebar - click "Lớp 10" active đúng lớp', async ({ page }) => {
    ...
    // Active text có thể là "Lớp 10" hoặc chứa "10"
