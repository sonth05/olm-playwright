...
/**
 * Map từ postman/collections/OLM-vn/Payment - Mua VIP/
 * - Get Mua VIP Page         GET /mua-vip                  (public)
 * - Get Gio Hang             GET /gio-hang                 (cần đăng nhập)
 * - Get Gio Hang Thu Vien So GET /gio-hang-thu-vien-so      (cần đăng nhập)
 */
...
test.describe('API @api Payment - Mua VIP', () => {
  ...
  test('Get Gio Hang - chưa đăng nhập', async ({ request }) => {
    ...
    // TODO xác nhận thực tế: redirect 302 về /dangnhap hay render 200 giỏ hàng rỗng.
    if (res.status() === 302) {
  ...
  // Sanity check riêng cho GIO_HANG_THU_VIEN_SO_URL vừa thêm vào config.ts,
  // đảm bảo build đúng URL (https://olm.vn/gio-hang-thu-vien-so).
