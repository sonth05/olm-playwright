 ...
 * - Get Login Page    GET  /dangnhap
 * - Get Register Page GET  /dang-ky
 * - Login              POST /dangnhap   (urlencoded: username, password)
 * - Register            POST /dang-ky    (urlencoded: name, username, email, password, phone)
 *
 * `request` fixture của Playwright dùng baseURL từ playwright.config.ts
 * (đã set sẵn = BASE_URL), nên có thể gọi path tương đối.
 */
...
test.describe('API @api Authentication', () => {
  ...
  test('Login với username/password rỗng không thành công', async ({ request }) => {
    ...
    // Cần xác nhận thực tế: OLM render lại 200 kèm box-error, hay redirect 302.
    // Tạm chấp nhận cả 2 cho tới khi verify qua DevTools (xem README-api.md).
