...
import { REGISTER_DATA, REGISTER_TEST_CASES } from '../src/config/testData';

test.describe('Registration @registration @regression', () => {
  ...
  test('[Unhappy] Bỏ trống họ tên', async ({ page }) => {
    ...
    expect(
      (await registerPage.hasValidationMessage()) || !registerPage.isRegistrationSuccessful()
    ).toBeTruthy();
  });
  ...
  test('[Unhappy] Email trùng', async ({ page }) => {
    ...
    await registerPage.clickSubmit();
    expect(
      (await registerPage.isEmailExistsModalShown()) ||
        (await registerPage.hasValidationMessage()) ||
  ...
  test('[Unhappy] Form trống hoàn toàn', async ({ page }) => {
    const registerPage = new RegisterPage(page);
    await registerPage.open();
    await registerPage.selectStudentAccount();
    await registerPage.clickSubmit();
  ...
  test('[Unhappy->Happy] Giữ dữ liệu sau submit lỗi', async ({ page }) => {
    const registerPage = new RegisterPage(page);
    const data = REGISTER_TEST_CASES.empty_email;
    await registerPage.open();
    await registerPage.selectStudentAccount();
