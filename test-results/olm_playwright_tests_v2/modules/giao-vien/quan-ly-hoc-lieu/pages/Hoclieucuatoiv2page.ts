...
export class ExamModal {
  ...
  constructor(page: Page) {
    ...
    // FIX: locator CŨ tìm button qua accessible NAME chứa placeholder text
    // (getByRole('button', { name: /Chọn khối lớp/i })). Với tài khoản/luồng
    // nào mà Khối lớp ĐÃ có sẵn giá trị mặc định khác placeholder (VD "Tổng
    // hợp lớp" thay vì "Chọn khối lớp") thì tên accessible của nút đổi theo
    // giá trị đang hiển thị -> locator theo tên placeholder KHÔNG khớp được
    // nút nào cả -> click() treo tới khi timeout, modal đứng yên y hệt màn
    // hình bị "stuck" chưa chọn được Môn học dù đã submit title. Đổi sang
    // locator CẤU TRÚC (button[aria-haspopup="dialog"], thứ tự cố định
    // Khối lớp trước - Môn học sau trong DOM) — độc lập với giá trị đang
    // hiển thị trên nút, giống cách CreateMaterialModal (Createhoclieumenu.ts)
    // đã làm.
  ...
  async selectGrade(label: string | RegExp) {
    ...
    await this.exactOption(label).click();
  ...
  /**
   * Locator KHỚP CHÍNH XÁC 1 option trong popover Khối lớp/Môn học.
   *
   * FIX: `getByRole('option', { name: label })` cũ dùng regex KHÔNG neo
   * (VD /Toán/i) hoặc string không exact -> khớp substring, nên với "Toán"
   * sẽ trúng đồng thời "Toán", "BD Toán", "Thực hành Toán", "Phụ đạo Toán",
   * "Toán (tiếng Pháp)" (DOM thật của popover Môn học có rất nhiều môn tên
   * gần giống nhau). `.click()` trên locator khớp NHIỀU phần tử ném lỗi
   * strict-mode ngay lập tức; safeClick() bắt lỗi, thử lại 1 lần với
   * `force: true` nhưng vẫn khớp nhiều phần tử nên vẫn lỗi -> modal đứng
   * yên ở trạng thái "Chọn môn học" (chưa chọn được gì), trông như bị
   * "stuck" khi chạy có UI. Bug này KHÔNG chỉ xảy ra ở modal "Đề kiểm tra"
   * — CreateMaterialModal (dùng chung cho 11 loại học liệu còn lại) có
   * cùng lỗi, đã sửa tương tự.
   *
   * Sửa: luôn khớp CHÍNH XÁC — string dùng exact: true; RegExp tự neo
   * ^...$ nếu người gọi chưa neo sẵn.
   */
