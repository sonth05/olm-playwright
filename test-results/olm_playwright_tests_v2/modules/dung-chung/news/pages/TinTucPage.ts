...
export class TinTucPage extends BasePage {
  ...
  // =========================================================================
  // Bổ sung — dùng bởi tin-tuc.regression.spec.ts nhưng trước đây chưa được
  // định nghĩa trên class này (TS2339 "Property does not exist").
  // =========================================================================

  /**
   * Kiểm tra trang có breadcrumb nav hay không. Dùng chung selector với
   * các module khác trong dự án (VD: CayThuMucPage.BREADCRUMB) —
   * `nav[aria-label="breadcrumb"] .breadcrumb`.
   */
  async hasBreadcrumb(): Promise<boolean> {
    try {
      return await this.page
        .locator('nav[aria-label="breadcrumb"] .breadcrumb')
        .first()
  ...
  /** Lấy tiêu đề trang (thẻ &lt;title&gt; của document). */
  async getPageTitle(): Promise<string> {
    return (await this.title()).trim();
  }

  /** Tổng số trang hiển thị trên thanh phân trang hiện tại. */
  async getPaginationPageCount(): Promise<number> {
    const nums = await this.getPageNumbers();
    return nums.length;
  }

  /**
   * Điều hướng trực tiếp tới trang phân trang thứ `n` của
   * /chu-de-bai-viet/hoc-tap — dạng URL `.../hoc-tap/page-{n}`, theo đúng
   * pattern phân trang chung của dự án (xem thêm
   * `danhSachCauHoiDaXoaPageUrl()` trong config.ts).
   */
  async gotoPage(n: number): Promise<this> {
    await this.navigateTo(`${TinTucPage.HOC_TAP_URL}/page-${n}`);
    await this.ensureSectionsLoaded();
    return this;
  }
}